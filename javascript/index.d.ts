 ...
 *
 * We do this by referring to a non-existent "vodozemac.js" file. This works because TSC will automatically
 * transform this into the correct name "vodozemac.d.ts" [1].
 *
 * Other alternatives don't work:
 *
 *  - `export * from "./pkg/vodozemac.d.ts"`: You're not allowed to `import from "<foo>.d.ts"`
 ...
 *
 *  - `export type * from "./pkg/vodozemac.d.ts"`: Doing this means you can't call the constructors on any
 *    of the exported types: `new RoomId(...)` gives a build error.
 *
 *  - `export * from "./pkg/vodozemac"`: This works in *some* environments, but in others, typescript
 ...
 *
 * A final alternative would be just to cat `vodozemac_bg.d.ts` in here as part of the build script, but
